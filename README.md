...
> **Status: Phase 0 (specification + foundation) complete.** No protocol contracts are
> implemented yet. This is not audited software and must not be used with real funds.
...
```bash
forge fmt && forge test -vvv && forge snapshot && forge coverage --ir-minimum --report summary
```
